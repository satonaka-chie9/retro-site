...
2. `render.yaml` を使用した「Blueprint Instance」としてデプロイするか、手動で以下の設定を行います：
   ...
   -  Renderでの設定方法
     Renderの管理画面で以下の環境変数を追加してください：
      * SUPABASE_URL: SupabaseプロジェクトのURL
      * SUPABASE_KEY: Supabaseプロジェクトの anon または service_roleキー
      * SUPABASE_BUCKET: 作成したバケット名（デフォルトはretro-site-uploadsにしています。Supabase側でこの名前のバケットを作成し、Publicに設定してください）
